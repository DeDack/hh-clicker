...
from app.models import HhSessionPayload
...
def classify_apply_response(status_code: int, text: str) -> tuple[str, dict]:
    ...
    if "alreadyApplied" in text or "вы откликнулись" in lowered or "отклик уже отправлен" in lowered or "отклик отправлен" in lowered:
        return "already", info
...
def send_apply(session: HhSessionPayload, resume_id: str, vacancy_id: str, cover_letter: str) -> tuple[str, dict]:
